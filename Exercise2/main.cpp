...
int main()
{
    ...
    Matrix2d A1 {{5.547001962252291e-01, -3.770900990025203e-02},{8.320502943378437e-01, -9.992887623566787e-01}};   //= Matrix2d::Zero();
    Vector2d b1 {-5.169911863249772e-01, 1.672384680188350e-01};   //= Vector2d::Zero();
    ...
    Matrix2d A2 {{5.547001962252291e-01, -5.540607316466765e-01},{8.320502943378437e-01,-8.324762492991313e-01}};   //= Matrix2d::Zero();
    Vector2d b2 {-6.394645785530173e-04, 4.259549612877223e-04};   //= Vector2d::Zero();
    ...
    Matrix2d A3 {{5.547001962252291e-01, -5.547001955851905e-01},{8.320502943378437e-01,-8.320502947645361e-01}};   //= Matrix2d::Zero();
    Vector2d b3 {-6.400391328043042e-10, 4.266924591433963e-10};   //= Vector2d::Zero();
...
void TestSolution(const MatrixXd& A,
                  ...
                  double& errRelQR)
{
    errRelPALU = (solution - SolveSystemPALU(A,b)).norm() / solution.norm();     //numeric_limits<double>::max();
    errRelQR = (solution - SolveSystemQR(A,b)).norm() / solution.norm();     //numeric_limits<double>::max();
}
